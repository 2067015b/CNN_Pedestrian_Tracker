...
class NumpyArrayIterator_for_CUHK03(pre_image.Iterator):
    ...
    def __init__(self, f, train_or_validation = 'train', flag = 1, image_data_generator = None,
        ...
        self.f = f
        self.length = len(list(f['a'][train_or_validation].keys()))
        self.train_or_validation = train_or_validation
        self.flag = flag
        self.image_data_generator = image_data_generator
        super(NumpyArrayIterator_for_CUHK03, self).__init__(3000000, int(batch_size / 2), shuffle, seed)
    ...
    def __next__(self):
        ...
        for i, j in enumerate(index_array):
            ...
            ka,kb = np.random.choice(list(range(self.length)),2)
...
class ImageDataGenerator_for_multiinput(pre_image.ImageDataGenerator):
    ...
    def flow(self, f, train_or_validation='train', flag=0, batch_size=BATCH_SIZE, shuffle=True, seed=1217, **kwargs):
