...
import numpy as np
import pickle
from re_id.cuhk03.model import generate_model,compile_model
from re_id.cuhk03.data_preparation import ImageDataGenerator_for_multiinput
#from Config import *
...
def train(model,
    ...
    with h5py.File(h5_path,'r') as f:
        ...
        for i in range(train_num):
            ...
            model.fit_generator(Data_Generator.flow(f, flag=flag_train), one_epoch, epoch_num, validation_data=Data_Generator.flow(
                f, train_or_validation=which_val_data, flag=flag_val), nb_val_samples=nb_val_samples)
            Rank1s.append(round(cmc(model)[0],2))
            print(Rank1s)
            model.save_weights('/export/home/2067015b/Re_ID/CUHK03/weights/'+weights_name+'_'+str(i)+'.h5')
            model_json = model.to_json()
            weights_as_numpy = model.get_weights()

            with open('/export/home/2067015b/Re_ID/CUHK03/weights/'+weights_name+'.pickle', 'wb') as output_pickle:
                pickle.dump(weights_as_numpy, output_pickle)

            model.save('/export/home/2067015b/Re_ID/CUHK03/weights/'+weights_name+'_full.h5')

            with open('/export/home/2067015b/Re_ID/CUHK03/weights/'+weights_name+'.h5', 'w') as f:
                f.write(model_json)
            model.load_weights(weights_as_numpy)
        return Rank1s
...
def _get_test_data(val_or_test='test'):
    with h5py.File('/export/home/2067015b/Re_ID/CUHK03/cuhk-03c.h5','r') as ff:
